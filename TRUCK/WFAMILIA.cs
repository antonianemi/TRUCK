...
{	
	public class WFAMILIA : Form1
    {
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(WFAMILIA));
		...
		private bool Find_Numero(string codigo)
		{
			...
			DataRow[] dr = dt.Tables[0].Select("familia = "+Convert.ToInt32(codigo));
			if (dr.Length > 0)
			{
				encontro = true;
				cmRegister.Position = buscar_posicion(Convert.ToInt32(codigo),"familia");
			}
			return encontro;
		}
		private bool Find_Nombre(string codigo)
		{
			...
			{
				encontro = true;
				cmRegister.Position = buscar_posicion(Convert.ToInt32(dr[0]["familia"]),"familia");
			}
			return encontro;
		}
        ...
        private void Mostrar_Datos(int pos, int op)
        {
            if (dt.Tables[0].Rows.Count > 0 && pos >= 0)
            {
                ...
                numero.Text = dr["familia"].ToString();
		...
		private void WFAMILIA_Close(object sender, CancelEventArgs e)
		{
			//Conec.Desconectar();			
			this.Close();
		...
		private void numero_Validating(object sender, System.ComponentModel.CancelEventArgs e)
		{
			if (this.numero.Text == "" )
			{
                IDataReader LP = db.getDataReader("SELECT familia FROM familia WHERE (numemp = " + Global.nempresa + ") ORDER BY familia desc");
		...
		private void toolBar1_ButtonClick(object sender, System.Windows.Forms.ToolStripItemClickedEventArgs e)
		{
            ...
            switch (this.toolBar1.Items.IndexOf(e.ClickedItem))
			{
				...
				case 1 : // Guardar Familia
				{
                    ...
                    {
                        this.editar_dato = false;
                         IDataReader df = db.getDataReader("SELECT numemp,familia FROM familia WHERE numemp = " + Global.nempresa + " and familia = " + Convert.ToInt32(this.numero.Text) + " ORDER BY familia");
				...
				case 3 :  // Cancelar
					...
					{					
						...
						if (cod != 0)this.numero.Text = Convert.ToString(Convert.ToInt32(dt.Tables[0].Rows[cod-1]["familia"].ToString())+1);
